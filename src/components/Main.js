...
class Result extends React.Component  {
  ...
  .then(response => {
      let results = response.data.results.map(x =>
        <div key ={x.language_code} className="div">
          <h2>{x.language_name}</h2>
          <p>Percentage: {x.percentage.toFixed(2)}</p>
          <p>Probability: {x.probability.toFixed(2)}</p>
        </div>
      )
  ...
  render() {
    ...
    return(
      ...
      <div className="center" >{this.state.result}</div>
    </div>
  )
  }
}
